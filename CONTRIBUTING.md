...
These are the rules we follow (from our own experience):

1. **Thin slice**: One change = one file = one commit. Verify immediately.
2. **Search before code**: Check if a solution exists before writing one.
3. **No blind fixes**: Never edit >2 times without verifying. If stuck, diagnose first.
4. **DEVLOG**: Record what you changed and why in `DEVLOG.md`.
5. **TypeScript**: `npx tsc --noEmit` must pass before every commit.
